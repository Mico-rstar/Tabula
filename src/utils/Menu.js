function createMenu(data, menuContainer, id, nodeData, key) {

    const menu = document.createElement('div');
    menu.className = 'mul-menu';

    function createMenuItem(item, level = 0) {
        const menuItem = document.createElement('div');
        menuItem.className = 'mul-menu-item';
        ...
        if (item.children && item.children.length > 0) {
            ...
            subMenu.className = 'mul-sub-menu';
            ...
            menuItem.addEventListener('click', () => {
                ...
                input.value = input.value + item.url + "}";
